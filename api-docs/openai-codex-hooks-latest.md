...
Current runtime values are `startup` and `resume`.

`UserPromptSubmit`

not supported

Any configured `matcher` is ignored for this event.
...
How the session started: `startup` or `resume`
...
This does not intercept all shell calls yet, only the simple ones. The newer `unified_exec` mechanism allows richer streaming stdin/stdout handling of shell, but interception is incomplete. Similarly, this does not intercept MCP, Write, WebSearch, or other non-shell tool calls.
...
Shell command Codex is about to run
...
Shell command Codex just ran

`tool_response`

`JSON value`

Bash tool output payload. Today this is usually a JSON string
...
User prompt that is about to be sent
